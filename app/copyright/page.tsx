...
export async function generateMetadata(): Promise<Metadata> {
  const document = (await listLegalDocuments()).find((item) => item.id === "copyright");
  return { title: document?.title || "版权声明", description: document?.description };
}

export default async function CopyrightPage() {
  const [documents, settings] = await Promise.all([listLegalDocuments(), getSiteSettings()]);
  const document = documents.find((item) => item.id === "copyright");
