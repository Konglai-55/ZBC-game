...
export async function generateMetadata(): Promise<Metadata> {
  const document = (await listLegalDocuments()).find((item) => item.id === "about");
  return { title: document?.title || "关于我们", description: document?.description };
}

export default async function AboutPage() {
  const [documents, settings] = await Promise.all([listLegalDocuments(), getSiteSettings()]);
  const document = documents.find((item) => item.id === "about");
